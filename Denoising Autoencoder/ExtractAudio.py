...
def get_audio(audio_url, file_name, start_trim, trim_duration):
    # Defining file names
    ...
    sox_command_trim = ['sox', file_name + '_new.wav', final_file + '.wav', 'trim', start_trim, trim_duration]
...
def get_RAW(file):
    rate, raw_data_0 = scipy.io.wavfile.read(file + '.wav')
...
def normalize_audio(audio_data):
    audio_data /= np.abs(np.max(audio_data))
    return audio_data
