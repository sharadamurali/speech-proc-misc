"""
Main file for the LMS N-step predictor
...
if __name__ == "__main__":
    ...
    raw_data_speech = ExtractAudio.get_RAW(speech_file_name)
    ...
    plt.plot(raw_data_speech[1:10000])
    ...
    for n in range(len(num_samples)):
        ...
        err_vec[n] = LMS.LMS_train(train_X, train_Y, N, mu, epoch)
        print('Done.')
    ...
    # Plotting noisy data
    plt.figure(3)
    plt.title('Noisy Audio segment')
    plt.xlabel('Samples')
    plt.ylabel('Amplitude')
    plt.plot(raw_data_noisy[1:10000])
    ...
    # Run the LMS predictor (train)
    print('Running the LMS predictor...')
    err_vec_noisy = LMS.LMS_train(train_X_noisy, train_Y_noisy, N, mu, epoch)
    print('Done')
